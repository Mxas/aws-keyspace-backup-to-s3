...
public class StringToSmallintCodec implements TypeCodec<String> {

	private final TypeCodec<Short> inner;

	StringToSmallintCodec() {
		this.inner = TypeCodecs.SMALLINT;
	}
	...
	public ByteBuffer encode(@Nullable String value, @NonNull ProtocolVersion protocolVersion) {
		return (value == null) ? null : inner.encode(Short.valueOf(value), protocolVersion);
	}

	@Nullable
	@Override
	public String decode(@Nullable ByteBuffer bytes, @NonNull ProtocolVersion protocolVersion) {
		Short decode = inner.decode(bytes, protocolVersion);
